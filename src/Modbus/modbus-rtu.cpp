...
int _modbus_rtu_prepare_response_tid(const unsigned char *req, int *req_length)
{
...
static int win32_ser_read(struct win32_ser *ws, unsigned char *p_msg,
...
ssize_t _modbus_rtu_send(modbus_t *ctx, const unsigned char *req, int req_length)
{
#if defined(_WIN32)
    //ctx_ modbus_rtu_t *ctx_rtu = ctx->backend_data;
    DWORD n_bytes = 0;
    return (WriteFile(ctx->w_ser.fd, req, req_length, &n_bytes, NULL)) ? n_bytes : -1;
#else
    return write(ctx->s, req, req_length);
#endif
}

ssize_t _modbus_rtu_recv(modbus_t *ctx, unsigned char *rsp, int rsp_length)
{
#if defined(_WIN32)
    //ctx_ return win32_ser_read(&((modbus_rtu_t *)ctx->backend_data)->w_ser, rsp, rsp_length);
    return win32_ser_read(&ctx->w_ser, rsp, rsp_length);
#else
...
int _modbus_rtu_check_integrity(modbus_t *ctx, unsigned char *msg,
...
//    if (pxMasterRtuCtx == ctx)
//    {
//        cout << "_modbus_rtu_check_integrity msg" << endl;
...
//    cout << "crc_calculated" << endl;
//    cout << "0x" << hex << uppercase << setw(2) << setfill('0') << (unsigned int)crc_calculated << " " << endl;
//    cout << "crc_received" << endl;
//    cout << "0x" << hex << uppercase << setw(2) << setfill('0') << (unsigned int)crc_received << " " << endl;
//    cout << "msg received" << endl;
//    for(int i=0; i<40; i++)
//    {
//        cout << "0x" << hex << uppercase << setw(2) << setfill('0') << (unsigned int)msg[i] << " " << endl;
//    }
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    speed_t speed;
#endif
    //ctx_ modbus_rtu_t *ctx_rtu = (modbus_rtu_t*)ctx->backend_data;
     ...
     */
    win32_ser_init(&ctx->w_ser);

    /* ctx->device should contain a string like "COMxx:" xx being a decimal
     * number */
    ctx->w_ser.fd = CreateFileA(ctx->device,
    ...
    /* Error checking */
    if (ctx->w_ser.fd == INVALID_HANDLE_VALUE)
    {
    ...
    /* Save params */
    ctx->old_dcb.DCBlength = sizeof(DCB);
    if (!GetCommState(ctx->w_ser.fd, &ctx->old_dcb))
    {
        fprintf(stderr, "ERROR Error getting configuration (LastError %d)\n",
                (int)GetLastError());
        CloseHandle(ctx->w_ser.fd);
        ctx->w_ser.fd = INVALID_HANDLE_VALUE;
        return -1;
    }
    ...
    /* Setup port */
    if (!SetCommState(ctx->w_ser.fd, &dcb))
    {
...
#if defined(_WIN32)
    /* Revert settings */
    if (!SetCommState(ctx->w_ser.fd, &ctx->old_dcb))
        fprintf(stderr, "ERROR Couldn't revert to configuration (LastError %d)\n",
                (int)GetLastError());

    if (!CloseHandle(ctx->w_ser.fd))
...
int _modbus_rtu_flush(modbus_t *ctx)
{
    ...
    //ctx_ modbus_rtu_t *ctx_rtu = ctx->backend_data;
    ctx->w_ser.n_bytes = 0;
    return (FlushFileBuffers(ctx->w_ser.fd) == FALSE);
#else
                       ...
                       struct timeval *tv, int length_to_read)
{
    int s_rc;
#if defined(_WIN32)
    //ctx_ s_rc = win32_ser_select(&(((modbus_rtu_t*)ctx->backend_data)->w_ser),
    length_to_read, tv);
    if (s_rc == 0)
{
    errno = ETIMEDOUT;
...
//    {
//        cout << "modbus_receive select ctx->s+1" << endl;
//        cout << "0x" << hex << uppercase << setw(2) << setfill('0') << (unsigned int)(ctx->s+1) << " " << endl;
//    }
    ...
    while ((s_rc = select(ctx->s+1, rfds, NULL, NULL, tv)) == -1)
    {
        if (pxSlaveRtuCtx == ctx)
        {
//            cout << "modbus_receive select while" << endl;//?
        }
        ...
        if (errno == EINTR)
        {
            ...
            if (pxSlaveRtuCtx == ctx)
            {
                cout << "modbus_receive rtu select errno == EINTR" << endl;//?
            }

            if (pxSlaveHmiRtuCtx == ctx)
            {
                cout << "modbus_receive rtu hmi select errno == EINTR" << endl;//?
            }
        ...
        else
        {
            if (pxSlaveRtuCtx == ctx)
            {
                cout << "modbus_receive select return -1" << endl;//?
            }
            return -1;
        }
    }
//    cout << "select pass" << endl;
    if (s_rc == 0)
    {
        /* Timeout */
        errno = ETIMEDOUT;
//        if (pxSlaveRtuCtx == ctx)
//        {
//            cout << "modbus_receive select errno = ETIMEDOUT" << endl;//?
//        }
        return -1;
    }
#endif
//    if (pxSlaveRtuCtx == ctx)
//    {
//        cout << "modbus_receive select return s_rc" << endl;//?
//    }
return s_rc;
}
...
modbus_t* modbus_new_rtu(modbus_t *ctx, const char *device,
                         int baud, char parity, int data_bit,
                         int stop_bit)
{
    //ctx_ modbus_t *ctx;
    //ctx_ modbus_rtu_t *ctx_rtu;
    size_t dest_size;
    size_t ret_size;

    //ctx_ ctx = (modbus_t *) malloc(sizeof(modbus_t));
//    ctx = &xSlaveRtutx;
//    pxSlaveRtuCtx = ctx;
    ...
    //ctx_ ctx->backend_data = (modbus_rtu_t *) malloc(sizeof(modbus_rtu_t));
    //ctx_ ctx_rtu = (modbus_rtu_t *)ctx->backend_data;

    dest_size = sizeof(ctx->device);
    ret_size = strlcpy(ctx->device, device, dest_size);
    if (ret_size == 0)
    {
