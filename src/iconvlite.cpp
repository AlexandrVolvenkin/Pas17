...
//-----------------------------------------------------------------------------------------------------
char* Cp1251ToUtf8(char *out, const char *in)
{
    static const int32_t table[128] =
    {
        0x82D0,0x83D0,0x9A80E2,0x93D1,0x9E80E2,0xA680E2,0xA080E2,0xA180E2,
        0xAC82E2,0xB080E2,0x89D0,0xB980E2,0x8AD0,0x8CD0,0x8BD0,0x8FD0,
        0x92D1,0x9880E2,0x9980E2,0x9C80E2,0x9D80E2,0xA280E2,0x9380E2,0x9480E2,
...
#define MAX_CP1251_STRING_LENGTH 1024
#define MAX_UTF8_SYMBOL_LENGTH 3

    char acTemp[MAX_CP1251_STRING_LENGTH * MAX_UTF8_SYMBOL_LENGTH];
    char *pcTemp;
    int32_t i16Index;
    ...
    while (*in)
    {
        if (i16Index < MAX_CP1251_STRING_LENGTH)
        {
            if (*in & 0x80)
            {
                int32_t v = table[(uint8_t)(0x7f & *in++)];
                if (v)
                {
                    *pcTemp++ = (char)v;
                    *pcTemp++ = (char)(v >> 8);
...
//-----------------------------------------------------------------------------------------------------
char* Utf8ToCp1251(char *out, const char *in)
{
    size_t len = strlen(in);
    convert_utf8_to_windows1251(in, out, len);
    return out;
}
