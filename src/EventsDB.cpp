...
//    printf("Callback argc %d\n\r", (argc));
    ...
    {
//        printf("Callback strlen(argv[i] %d\n\r", (strlen(argv[i])));
//        printf("Callback argv %s\n\r", (argv[i]));
    ...
    // Return successful.
//    printf("Callback ui8QueryGetRowQuantity %d\n\r", (ui8QueryGetRowQuantity));
    return 0;
}
...
//        cout << "SQLite error2: " << zErrMsg << endl;
        if (rc != SQLITE_OK)
        {
...
//
//Если вы хотите, чтобы SQL - IGNORE эта ошибка и продолжили добавлять другие записи , сделайте это :
//
//INSERT or IGNORE into tablename VALUES (value1,value2 , so on );
//
//Если вы хотите заменить значения в таблице всякий раз , когда запись уже существует, сделайте это:
//
//INSERT or REPLACE into tablename VALUES (value1,value2 , so on );
//
//Это экономит много обработки с вашей стороны и довольно полезно.


//    SendQuery("attach '/home/debian/Pas05ArchiveDataBasePrevious.db' as toMerge;");
//    SendQuery("BEGIN;");
//    SendQuery("insert or IGNORE into Events select * from toMerge.Events;");
//    SendQuery("COMMIT;");
//    SendQuery("detach toMerge;");
//
//    sprintf(acCommand,
//            "%s %s %s",
//            "sudo cp -f",
//            pccEventsDataBaseName,
//            pccEventsDataBaseNameMerge);

//    Close();
//
////    // создадим команду удалим предыдушую часть архива.
////    sprintf(acCommand,
////            "%s %s",
////            "sudo rm -f",
////            pccEventsDataBaseNamePrevious);
////    system(acCommand);
//    // создадим команду копируем текущую часть архива в предыдущую.
////    sprintf(acCommand,
////            "%s %s %s",
////            "sudo mv -f",
////            pccEventsDataBaseName,
...
//    system(acCommand);
//
//    // создадим команду удалим текущую часть архива.
//    sprintf(acCommand,
//            "%s %s",
//            "sudo rm -f",
//            pccEventsDataBaseName);
...
//            sprintf((char*)acQuery,
//                    "%s '%d' %s '%d' %s '%d' %s %s '%d';",
//                    "SELECT * FROM Events WHERE EventAddress ==",
...
//            sprintf((char*)acQuery,
//                    "%s '%s' %s '%d' %s %s '%d';",
//                    "SELECT * FROM Events WHERE EventDate ==",
//                    accDateStr,
//                    "AND id <",
//                    ui16LastFirstID,
//                    "ORDER BY id DESC",
