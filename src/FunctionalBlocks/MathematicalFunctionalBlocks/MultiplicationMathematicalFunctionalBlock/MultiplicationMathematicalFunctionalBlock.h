#ifndef CMULTIPLICATIONMATHEMATICALFUNCTIONALBLOCK_H
#define CMULTIPLICATIONMATHEMATICALFUNCTIONALBLOCK_H
//-----------------------------------------------------------------------------------------------------
//  Source      : FileName.cpp
//  Created     : 01.06.2022
...
//struct TMultiplicationMathematicalFunctionalBlockDescriptor
//{
...
//-----------------------------------------------------------------------------------------------------
class CMultiplicationMathematicalFunctionalBlockJob :
	public CGeneralMathematicalFunctionalBlockJob
{
public:

	CMultiplicationMathematicalFunctionalBlockJob();
	virtual ~CMultiplicationMathematicalFunctionalBlockJob();
...
//	// если бит 7 этого реквизита = 1,
//	// то биты 0-6 – это номер расчетной величины CALC_VAL_ADDR (1-48).
//	// Если адрес одной из переменных = 0, то операция выполняется с оставшейся переменной, если адреса обеих переменных = 0, то результат равен константе С.
...
//-----------------------------------------------------------------------------------------------------
class CMultiplicationMathematicalFunctionalBlockManager :
	public CGeneralMathematicalFunctionalBlockManager
{
public:

	CMultiplicationMathematicalFunctionalBlockManager();
	virtual ~CMultiplicationMathematicalFunctionalBlockManager();
//    virtual void Fsm(void);
...
//-----------------------------------------------------------------------------------------------------
class CMultiplicationMathematicalFunctionalBlockExecutor :
	public CGeneralMathematicalFunctionalBlockExecutor
{
public:

	CMultiplicationMathematicalFunctionalBlockExecutor();
	virtual ~CMultiplicationMathematicalFunctionalBlockExecutor();
	virtual void Fsm(void);
