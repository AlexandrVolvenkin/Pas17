...
//-------------------------------------------------------------------------------
// обрабатывает вход€щие сообщени€ от Modbus интерфейсов по 69 функции - запись блока базы данных.
uint16_t CDeviceControl::DataBaseBlockWrite(void)
{
    ...
    // номер прин€того дл€ записи блока базы данных.
    switch(uiBlockIndex)
    {
...
//                ((CDataContainerDataBase*)GetCustomerDataContainerPointer()) -> m_uiFsmCommandState = DONE_ERROR;
    ...
    case DATA_BASE_BLOCK_START_WRITE:
        std::cout << "CDeviceControl::Fsm DATA_BASE_BLOCK_START_WRITE"  << std::endl;
        {
                               ...
                               ((CDataContainerDataBase*)GetCustomerDataContainerPointer()) -> m_uiDataIndex))
            {
                ...
                // ”становим врем€ ожидани€ окончани€ записи.
                GetTimerPointer() -> Set(TASK_READY_WAITING_TIME);
    ...
    case DATA_BASE_BLOCK_WRITE_END_WAITING:
        ...
        // ќжидаем окончани€ записи автоматом устройства хранени€.
        // ”стройство хранени€ закончило запись успешно?
        if (m_pxDataStore ->
                IsDoneOk())
        {
...
//            SetFsmState(DATA_BASE_BLOCK_MODULE_MUVR_WRITE_START);
        ...
        {
            // ¬рем€ ожидани€ окончани€ записи закончилось?
            if (GetTimerPointer() -> IsOverflow())
            {
